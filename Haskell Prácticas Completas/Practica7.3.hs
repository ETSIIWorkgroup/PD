...
vacio :: MultiConj a
vacio = M.empty         -- Usamos 'M' Data.Map está importado como 'M'.
...
unitario :: a -> MultiConj a
unitario conjunto = M.singleton conjunto 1 
-- (singleton k x) es el diccionario cuya única clave es 'k' y su valor es 'x'.
...
inserta :: Ord a => a -> MultiConj a -> MultiConj a
inserta elemento = M.insertWith (+) elemento 1
--                 diccionario  operacion  elemento valor

-- (insertWith f k x d) es el diccionario obtenido añadiéndole a d el par (k,x) 
-- si k no es una clave de d o el par (k,f x y) si el par (k,y) pertenece a d. 
...
-- Operador y zipea la lista con tantos '1' como veces aparece el elemento
...
-- Igual que 'inserta', pero ahora en lugar de poner '1', ponemos 'repeticiones'
insertaVarios :: Ord a => a -> Int -> MultiConj a -> MultiConj a
insertaVarios elemento repeticiones = M.insertWith (+) elemento repeticiones
...
borra :: Ord a => a -> MultiConj a -> MultiConj a
borra elemento = M.update f elemento
    where f c | c <= 1    = Nothing         -- Si hay 1 o menos, devuelve nada
              | otherwise = Just (c - 1)    -- En otro caso, resta 1 al nº de ocurrencias
...
borraVarias :: Ord a => a -> Int -> MultiConj a -> MultiConj a
borraVarias elemento n = M.update (f n) elemento
    where f n c | c <= 1       = Nothing    
                | (c - n) <= 1 = Nothing
                | otherwise    = Just (c - n)
...
borraTodas :: Ord a => a -> MultiConj a -> MultiConj a
borraTodas elemento = M.delete elemento
...
pertenece :: Ord a => a -> MultiConj a -> Bool
pertenece elemento = M.member elemento
...
noPertenece :: Ord a => a -> MultiConj a -> Bool
noPertenece elemento = not . pertenece elemento
...
ocurrencias :: Ord a => a -> MultiConj a -> Int
ocurrencias elemento = M.findWithDefault 0 elemento
...
esSubmultiConj :: Ord a => MultiConj a -> MultiConj a -> Bool
esSubmultiConj m1 m2 =     all (\x -> ocurrencias x m1 <= ocurrencias x m2)
        (elementos m1)
